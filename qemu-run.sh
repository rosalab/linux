...
INNER_UNIKERNELS_PATH=/home/raj/inner_unikernels
LINUX_PATH=`readlink -f ~/6_linux/linux-6.0.2`
...
sudo killall qemu-system-x86_64 2>/dev/null

sudo $QEMU \
     -machine accel=kvm:tcg \
     -enable-kvm -cpu host -m 256m -smp 2 \
     -kernel ${KERNEL} -append "earlyprintk=ttyS0 console=ttyS0 reboot=k nomodules panic=1 root=/dev/vda init=/sbin/init nokaslr" \
