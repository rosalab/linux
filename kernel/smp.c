...
static void flush_smp_call_function_queue(struct pt_regs *regs, bool warn_cpu_offline);
...
int smpcfd_dying_cpu(unsigned int cpu)
{
	 ...
	 * still pending.
	 */
	flush_smp_call_function_queue(task_pt_regs(current), false);
	irq_work_run();
	return 0;
}
...
void generic_smp_call_function_single_interrupt(struct pt_regs *regs)
{
	...
	flush_smp_call_function_queue(regs, true);
}
...
void flush_smp_call_function_from_idle(struct pt_regs *regs)
{
	unsigned long flags;
